...
$duplicatePaths = @($declaredPaths | Group-Object | Where-Object { $_.Count -gt 1 } | ForEach-Object { $_.Name })
...
Write-ModuleLog "Schema and invariants: $($declaredKeys.Count) job(s), $(@($declaration.folders).Count) folder(s), maximumDepth $($declaration.maximumDepth). Valid."
...
foreach ($path in $pathsToRead) {
    try {
        ...
        Write-ModuleLog "could not read '$path': $($_.Exception.Message)"
    }
}
