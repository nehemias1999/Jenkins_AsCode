...
function New-ProjectContextFixture {
    <#
    .SYNOPSIS
        An invented project context, for building a request context without reading
        the repository's own configuration.

    .EXAMPLE
        New-ProjectContextFixture
    ...
    return [pscustomobject]@{
        jenkins  = [pscustomobject]@{
            baseUrlEnv = 'TEST_JENKINS_URL'
            userEnv    = 'TEST_JENKINS_USER'
            tokenEnv   = 'TEST_JENKINS_API_TOKEN'
        }
        jira     = [pscustomobject]@{
            baseUrlEnv = 'TEST_JIRA_BASE_URL'
            emailEnv   = 'TEST_JIRA_EMAIL'
            tokenEnv   = 'TEST_JIRA_API_TOKEN'
            apiVersion = '3'
        }
